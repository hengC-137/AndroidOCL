...
__kernel void myGEMM6(const int M, const int N, const int K, const __global float* A,
    ...
    for (int k = 0; k < K; k++){
        av = vload4(0, &A[globalRow * 4 * K + 4 * k]);
        bv = vload4(0, &B[page + (k << 4) + (globalCol & 3)]);
...
__kernel void myGEMM7(const int M, const int N, const int K, const __global float* A,
    ...
    const __global float* ptrA = A + globalRow * 4 * K;
    const __global float* ptrB = B + (globalCol >> 2) * (K >> 4) + (globalCol & 3);
